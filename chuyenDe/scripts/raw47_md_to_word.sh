...
# Regenerate DOCX from existing Markdown only (no re-splitting raw47, no reference rebuild).
...
if [[ ! -d "${md_root}" ]]; then
  ...
  echo "Generate it via: python3 ${repo_root}/chuyenDe/scripts/raw47_to_markdown.py" >&2
  exit 1
fi

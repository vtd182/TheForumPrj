...
def _find_raw47_input_dir(repo_root: Path) -> Path:
    p = repo_root / "chuyenDe" / "RW" / "raw" / "rawr8"
    if p.exists():
        return p
    raise SystemExit(f"Cannot find rawr8 input dir: {p}")


WEEK_RE = re.compile(r"w(?P<week>\d+)", re.IGNORECASE)


def main() -> int:
    parser = argparse.ArgumentParser(description="Split rawr8 docx into Markdown files (Reading).")
    parser.add_argument(
        "--out",
        default="chuyenDe/RW/md/rawr8",
        help="Output dir for Markdown (default: chuyenDe/RW/md/rawr8)",
    )
    ...
    for docx_path in docx_files:
        m_level = LEVEL_RE.search(docx_path.name)
        if not m_level:
            raise SystemExit(f"Cannot infer level from filename: {docx_path.name}")
        level = int(m_level.group("level"))

        m_week = WEEK_RE.search(docx_path.name)
        if not m_week:
            raise SystemExit(f"Cannot infer week from filename: {docx_path.name}")
        week = int(m_week.group("week"))

        paragraphs = extract_paragraphs(docx_path)

        # rawr8 files are standalone Reading passages without a "W7 (dates): Reading" header.
        # Treat the entire file as a single Reading section.
        section = Section(week=week, dates="", skill="Reading", paragraphs=paragraphs)
        md = render_reading_md(level=level, section=section)

        out_path = out_paths.md_path(skill="Reading", level=level, week=week)

...
def _find_raw47_input_dir(repo_root: Path) -> Path:
    p = repo_root / "chuyenDe" / "RW" / "raw" / "raww9"
    if p.exists():
        return p
    raise SystemExit(f"Cannot find raww9 input dir: {p}")


def main() -> int:
    parser = argparse.ArgumentParser(description="Split raww9 docx into Markdown files (Writing).")
    parser.add_argument(
        "--out",
        default="chuyenDe/RW/md/raww9",
        help="Output dir for Markdown (default: chuyenDe/RW/md/raww9)",
    )

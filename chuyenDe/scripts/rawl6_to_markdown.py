...
def main() -> None:
    ap = argparse.ArgumentParser(description="Convert rawl6 DOCX (Listening/Speaking) into Markdown for templates.")
    ap.add_argument("--in-dir", type=Path, default=Path("chuyenDe/LS/raw/rawl6"))
    ap.add_argument("--out-dir", type=Path, default=Path("chuyenDe/LS/md/rawl6"))
    args = ap.parse_args()
